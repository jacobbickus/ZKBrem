...
G4NRFNuclearLevelStore::G4NRFNuclearLevelStore()
{
    ...
    for (int a = 1; a <= 300; a++)
      for (int z = 1; z <= 100; z++){
        ...
        //G4cout << GetKeyIndex(z,a) << " " << theKeys_fast[zz++] << G4endl;
      }
}
...
G4NRFNuclearLevelManager* G4NRFNuclearLevelStore::GetManager(const G4int Z, const G4int A, G4bool standalone)
{
  ...
  //if (theKeys_fast[index] != NULL)
    //G4cout << "key = " << key << "; index = " << index << G4endl;//"; theKeys_fast[index] = " << theKeys_fast[index] << G4endl;
  //G4cout << "index = " << index << "; key = " << key << G4endl;
    
  ////// Check if already exists that key
  //std::map<G4String,G4NRFNuclearLevelManager*>::iterator idx = theManagers.find(key);
  //// If doesn't exist then create it
  //if ( idx == theManagers.end() )
  //{
  //  if (debug) G4cout << "G4NRFNuclearLevelStore::GetManager -- Creating new level manager for Z=" << Z << " A=" << A << G4endl;
