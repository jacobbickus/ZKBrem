...
G4NRFNuclearLevelManager::G4NRFNuclearLevelManager(const G4int Z, const G4int A, const G4String& filename,
						   G4bool Verbose) :
...
void G4NRFNuclearLevelManager::SetNucleus(const G4int Z, const G4int A, const G4String& filename, G4bool standalone)
{
    if (_nucleusZ != Z || _nucleusA != A)
    {
      ...
      if (_Verbose) {
        G4cout << "G4NRFNuclearLevelManager::SetNucleus Back from MakeLevels"
	             << G4endl;
      }
...
void G4NRFNuclearLevelManager::ReadGroundStateProperties(G4bool standalone) 
{
  ...
  if (debug) G4cout << "----------Reading new ground state properties----------" << G4endl;
  ...
  while ((!found_gs) && (!gsFile.eof())) {
    ...
    if (debug) G4cout << std::setw(6) << "Z = " << std::setw(3) << Z 
                      << std::setw(8) << "A = " << std::setw(3) << A
                      << std::setw(12) << "spin = " << std::setw(4) << spin
                      << std::setw(12) << "parity = " << std::setw(2) << parity
                      << G4endl;
...
void G4NRFNuclearLevelManager::ReadTDebyeData(G4bool standalone)
{
  ...
  if (debug) G4cout << "----------Reading new T_Debye values----------" << G4endl;
  ...
  while ((!found_TD) && (!TDFile.eof())) {
    ...
    if (Z == _nucleusZ)
    {
      if      (TDA == 0 and TDK == 0) TDebye_tmp = 0.0;
      else if (TDA == 0 and TDK != 0) TDebye_tmp = TDK;
      else if (TDA != 0 and TDK == 0) TDebye_tmp = TDA;
      else if (TDA != 0 and TDK != 0)
      {
        if      ( LA and !LK) TDebye_tmp = TDK;
        else if (!LA and  LK) TDebye_tmp = TDA;
        else TDebye_tmp = (TDA + TDK)/2.0;
      }
      else {G4cout << "Error in TDebye decision" << G4endl; exit(7);}
      found_TD = true;
    }
  }
  if (debug) G4cout << "Z = " << Z << ", TD = " << TDebye_tmp << G4endl;
  ...
  else SetTDebye(TDebye_tmp*kelvin);
}
...
const G4NRFNuclearLevel* G4NRFNuclearLevelManager::
NearestLevelRecoilAbsorb(const G4double GammaEnergy, const G4double eDiffMax)
{
  ...
  const G4double E_recoil = GammaEnergy*GammaEnergy/(2.0 * M);
...
NearestLevel(const G4double energy, const G4double eDiffMax)
{
  ...
  if (_levels != 0)
  {
    ...
    for (i=0; i<numLevels; ++i)
    {
      ...
      if (eDiff > eDiffLast and i != 0) break;
      eDiffLast = eDiff;
    }
  }
  ...
  { 
    //G4cout << "energy/MeV = " << energy/MeV << ", level chosen [MeV]: " << _levels->operator[](iNear)->Energy()/MeV << G4endl;
  ...
  if (dataFile >> _levelEnergy)
    {
      ...
      if (debug) G4cout << "----------_gammaEnergy = " << _gammaEnergy/MeV << " MeV." << G4endl;
    ...
    if (debug) G4cout << "----------Finished call to G4NRFNuclearLevelManager::Read()----------" << G4endl;
...
void G4NRFNuclearLevelManager::delete_bad_levels()
{
  ...
  for(int i = 0; i < _levels->size(); i++)
  {
    ...
    //G4cout << "Level 21 energy = " << (_levels->operator[](21))->Energy()/MeV << " MeV." << G4endl;
    G4bool invalidLevel = checkLevel->GetInvalidLevel(); // check whether the level is invalid
    G4double e = checkLevel->Energy()/MeV;
    //G4cout << "invalidLevel = " << invalidLevel << G4endl;
    if (invalidLevel)
    {
      //G4cout << "Deleting bad level " << i << " at " << checkLevel->Energy()/MeV << " MeV." << G4endl;
      _levels->erase(_levels->begin()+i); // if so, delete it
      if (e > 2.175 and e < 2.177 and checkLevel->Z() == 92)
      {
        G4cout << "Deleting level with energy " << e << " MeV." << G4endl;
        exit(1);
      }
...
void G4NRFNuclearLevelManager::SetTeff(G4double Teff)
{ _Teff = Teff; }


G4double G4NRFNuclearLevelManager::GetTeff()
{ return _Teff; }
