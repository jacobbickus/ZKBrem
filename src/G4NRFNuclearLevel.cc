...
//    spins using the deleteUnknownLevels boolean. If the spin of the level is
//    unknown, it would previously get set to -999 and cause an exit() error.
//    Now, by using Get/SetInvalidLevel() of G4NRFNuclearLevel, we can mark the
//    level as invalid, then in G4NRFNuclearLevelManager, we use the function
//    delete_bad_spins() to erase such levels in their entirety.
//
...
const bool debug = false;               // production runs use false
const bool deleteUnknownLevels = true;  // production runs use true

const double EDIFF_TOL_gamma = 10.0*eV; // default is 10.0*eV, probably good enough
const double EDIFF_TOL_level = 10.0*eV;
...

{
  ...
  if (debug) G4cout << "G4NRFNuclearLevel is being called. energy (MeV): " << energy/MeV << G4endl;
...
G4double G4NRFNuclearLevel::Z() const
{
  return _nucleusZ;
}

// moved inline
//G4double G4NRFNuclearLevel::Energy() const
//{
//  return _energy;
//}
...
G4double G4NRFNuclearLevel::SelectGamma(G4int& igamma) const
{
  ...
  //if (debug) G4cout << "----------Calling SelectGamma()----------" << G4endl;
  ...
  //if (debug) G4cout << "----------Finished with SelectGamma()----------" << G4endl;
...
G4double G4NRFNuclearLevel::GetGSProb() const
{
  double GSprob = 0.0;
  for (int i=0; i<_nGammas; i++){
    //if (_energy/MeV > 2.2 && _energy/MeV < 2.3) G4cout << _energies[i] << " " << _energy << " " << fabs(_energies[i] - _energy) << " " << _prob[i] << G4endl;
    if (fabs(_energies[i] - _energy) <= EDIFF_TOL_level)
    {
      //G4cout << fabs(_energies[i] - _energy)/eV << " vs " << EDIFF_TOL_level/eV << G4endl;
      GSprob = _prob[i];
    }
  }
  return GSprob;
...
void G4NRFNuclearLevel::RefreshGammas() {
  ...
  //if (_Verbose)
	if (debug) G4cout << "RefreshGammas() is being called." << G4endl;
  ...
  if (debug) G4cout << "----------About to look for level in table----------" << G4endl;
  ...
  while ((!found_level) && GammaTableFile.getline(line,130)) {
    ...
    if (E_level <= -2.4e6) // deal with the bizarre lines with +X in NNDC
    {
      G4double E_init = E_level;
      E_level *= -1.0;
      E_level -= 2.4e6;
      if (debug) G4cout << "Altered E_level from " << E_init << " to " << E_level << " keV." << G4endl;
    }

    if (Z == _nucleusZ) {
      E_level *= keV;
      if (debug) G4cout << setw(8) << E_level/keV << setw(8) << _energy/keV << G4endl;
  ...
  // reluctant secondary check? - still causes other problems in Pu-239, Ge, Pb - might actually work then get to gamma errors
  G4bool secondCheck = false;
  if (!found_level and min_level_diff < 0.8*keV)
  {
    if (debug) G4cout << "Reluctant secondary check" << G4endl;
    found_level = true;
    secondCheck = true;
    E_level = closest_level;
  }
  ...
  if (debug) G4cout << "----------About to store list of info for the level----------" << G4endl;
  ...
  for (G4int jgamma=0; jgamma<num_gammas; jgamma++) {
    ...
    Egamma_arr[jgamma] *= (keV / (secondCheck? keV : 1.0)); // FIXME this is a kludge that works but I don't know why
  ...
  // found, the routine currently aborts.

  if (debug) G4cout << "----------About to match level gammas with multipole info----------" << G4endl;

  G4int i;
  //G4double Ediff_min = DBL_MAX; // these were poorly placed
  //G4int jgamma_min = 0;
  ...
  for (i=0; i<_nGammas; i++) {
    ...
    while ((!found_gamma) && (jgamma < num_gammas)) {
      ...
      if (Ediff <= EDIFF_TOL_gamma or Ediff/E_g <= 1.0e-3) {
        found_gamma = true; // only place found_gamma is set in Refresh_Gammas()

        if (_Verbose or debug) G4cout << "RefreshGammas: Found gamma (keV): " << E_g/keV << G4endl;
      }
      else jgamma++;
    } // end while loop
    ...
    if (debug) G4cout << "----------About to store multipole info for gamma----------" << G4endl;
    ...
    if (found_gamma) {
     ...
     if (debug) {
      G4cout << "Error in G4NRFNuclearLevel::RefreshGammas." << G4endl;
      G4cout << "Could not locate gamma in table." << G4endl; // reluctant secondary check fails here
      G4cout << "Looking for gamma (keV): " << E_g/keV << G4endl;
      G4cout << "Nucleus A: " << _nucleusA << " Z: " 
	     << _nucleusZ << G4endl;
      ...
      if (debug) G4cout << "----------found_gamma = " << found_gamma << "----------" << G4endl;
  ...
  if (debug) G4cout << "----------Finished looping over gamma levels----------" << G4endl;
}
...
void G4NRFNuclearLevel::RefreshWidth() {
  ...
  if (!env) 
   {
       G4cout << "G4NRFNuclearLevel: please set the G4NRFGAMMADATA environment variable"
	      << G4endl;
       G4cout << "Aborting." << G4endl;
       exit(35);
   }

  if (debug) G4cout << "----------Calling RefreshWidth()----------" << G4endl;
  ...
  while ((!found_level) && (!LevelsFile.eof())) {
    ...
    if ((Z == _nucleusZ) && (A == _nucleusA)) {
      ...
      if (Ediff <= EDIFF_TOL_level)
      {
        ...
        if (deleteUnknownLevels) // deal with levels that have empirically-unknown spins
        {
          if (spin == -999)
          {
            if (debug) G4cout << "spin = -999 at E_level = " << E_level << " keV." << G4endl;
            this->SetInvalidLevel();
            //G4cout << "Just called SetInvalidLevel(). GetInvalidLevel() = " << GetInvalidLevel() << G4endl;
          }
        }

        if (spin != _angularMomentum and !this->GetInvalidLevel()) {
          //G4cout << "Mismatch in angular momentum at E_level = " << E_level << "." << G4endl; // this was the error in Be, Iron, Glass, etc.
...
void G4NRFNuclearLevel::MakeWidth0() {
  ...
  if (debug) G4cout << "----------Calling MakeWidth0()----------" << G4endl;
...
void G4NRFNuclearLevel::Calc_Tau0() {
  ...
  if (debug) G4cout << "----------Calling Calc_Tau0()----------" << G4endl;
...
G4bool G4NRFNuclearLevel::Identify_GS_Transition() {
  ...
  if (debug) G4cout << "----------Calling Identify_GS_Transition()----------" << G4endl;
  ...
  // Nuclear recoil energy (non-relativistic approx.)
  ...
  if (debug) G4cout << "----------Finished with Identify_GS_Transition()----------" << G4endl;
